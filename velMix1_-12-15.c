...
/*Open fileS as readable/writable binary*/
	...
	while ( thdr.iline - iOrigin <= numTr/2){
   ...
   for (ii=0;ii< iMax-iOrigin + numTr/2;++ii){
		...
		* trGth[1] -> trGth[0], trGth[2] -> [1], and tempTr -> trGth[numTr-1]*/
		...
		for (i = xNdxMn[ilineCnt]; i<=xNdxMx[ilineCnt];++i){ 	
			...
			segyWriteTrace(foutVel, trGthHdr[numTr/2][i], velOutTr[numTr/2][i], ns, endian);	
		}
		}
	}
				...
				* Zero-Plus goes upward, and Zero-Minus goes downward*/
				...
				for(xlDirec = -1*1;xlDirec<=1; ++xlDirec){
					...
					* so, to avoid this I am skipping over ther case where both ilDirec and xlDirec are zero.*/
					...
					if( sampPlusWin >=0 && sampPlusWin < ns){
					//if (boundCheck (sampPlusWin, 0 , ns)==1){
					...
					//while (i<=halfTr && boundCheck(sampPlusWin,slopeDiag,ns) ==1 && semTr[halfTr +ilMove][xNdx + xlMove][sampPlusWin-slopeDiag] < thresh && slopeChk(slopeOld,slopeDiag,maxDip)==1){								
						...
						//while (i<=halfTr && boundCheck(sampPlusWin,slopeXl,ns)==1 && semTr[halfTr +ilMove][xNdx + xlMove][sampPlusWin-slopeDiag] < thresh && slopeChk(slopeOld,slopeDiag,maxDip)==1){
